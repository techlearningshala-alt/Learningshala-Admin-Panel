...
export default function CourseFaqInlinePanel({
  courseId,
  courseName,
  stagedFaqs,
  setStagedFaqs,
  ...
  const isExistingEntity = Boolean(courseId);
  ...
  const {
    ...
    queryKey: ["course-faq-inline", courseId],
    queryFn: () => fetchCourseFaqsByCourseId(courseId),
  ...
  const handleSaveFaq = async (values, { addAnother = false } = {}) => {
    ...
    if (!isExistingEntity) {
      if (editingFaq?._isRemote === false || editingFaq?.tempId) {
        ...
        notifySuccess("FAQ staged. It will be saved after the course is created.");
      }
      if (!addAnother) {
        handleCloseFaqForm();
      } else {
    ...
    try {
      if (editingFaq?._isRemote && editingFaq.id) {
        await updateCourseFaq(editingFaq.id, {
          ...finalPayload,
          course_id: courseId,
        });
      ...
      queryClient.invalidateQueries(["course-faq-inline", courseId]);
      if (!addAnother) {
        handleCloseFaqForm();
      } else {
        setEditingFaq(null);
