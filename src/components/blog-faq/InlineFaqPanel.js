...
export default function BlogFaqInlinePanel({
  blogId,
  blogName,
  stagedFaqs,
  setStagedFaqs,
  ...
  const {
    ...
    queryKey: ["blog-faq-inline", blogId],
    queryFn: () => fetchBlogFaqsByBlogId(blogId),
  ...
  const handleSaveFaq = async (values, { addAnother = false } = {}) => {
    ...
    if (!isExistingEntity) {
      if (editingFaq?._isRemote === false || editingFaq?.tempId) {
        ...
        notifySuccess("FAQ staged. It will be saved after the blog is created.");
      }
      if (!addAnother) {
        handleCloseFaqForm();
      } else {
    ...
    try {
      if (editingFaq?._isRemote && editingFaq.id) {
        await updateBlogFaq(editingFaq.id, {
          ...finalPayload,
          blog_id: blogId,
        });
      ...
      queryClient.invalidateQueries(["blog-faq-inline", blogId]);
      if (!addAnother) {
        handleCloseFaqForm();
      } else {
        setEditingFaq(null);
