...
const SECTION_TEMPLATES = [
  ...
  { id: "who-can-pursue", section_key: "who_can_pursue_1", title: "Who Can Pursue 1 ? (Box-1)" },
  { id: "who-can-pursue", section_key: "who_can_pursue_2", title: "Who Can Pursue 2 ? (Box-2)" },
  { id: "who-can-pursue", section_key: "who_can_pursue_3", title: "Who Can Pursue 3 ? (Box-3)" },
  { id: "who-can-pursue", section_key: "who_can_pursue_4", title: "Who Can Pursue 4 ? (Box-4)" },
...
export default function AddCourseForm({ item, onCancel, onSuccess }) {
  ...
  const { data: domainResponse, isLoading: isLoadingDomains } = useQuery({
    queryKey: ["domains", "all"],
    queryFn: () => fetchDomainsForCourse({ page: 1, limit: 200 }),
  });
  ...
  const { data: emiPartnerResponse } = useQuery({
    queryKey: ["emi-partners", "all"],
    queryFn: () => fetchAllEmiPartners(),
  });

  const domains = useMemo(
    () => domainResponse?.data?.data || domainResponse?.data || [],
    [domainResponse]
  );
  ...
  const emiPartners = useMemo(
    () =>
      emiPartnerResponse?.data?.data ||
      emiPartnerResponse?.data ||
      emiPartnerResponse ||
      [],
    [emiPartnerResponse]
  );
  ...
  const selectedEmiPartnersDisplay = useMemo(
    () =>
      emiPartners.filter((p) => selectedEmiPartners.includes(p.id)),
    [emiPartners, selectedEmiPartners]
  );

  const hydrateSections = useCallback((course) => {
    const base = buildDefaultSections();
    const remoteSections = Array.isArray(course?.sections)
      ? course.sections
      : [];
  ...
  const hydrateBanners = useCallback((course) => {
    const list = Array.isArray(course?.banners) ? course.banners : [];
  ...
  const applyCourseData = useCallback(
    (source) => {
      ...
      const hydratedValues = {
        ...defaultFormValues,
        domain_id: source.domain_id ? String(source.domain_id) : "",
        name: source.name || "",
        slug: source.slug || "",
        h1Tag: source.h1Tag || "",
      ...
      setSelectedEmiPartners(
        Array.isArray(source?.emi_partner_ids)
          ? source.emi_partner_ids.map((n) => Number(n)).filter((n) => !Number.isNaN(n))
          : []
      );
    },
  ...
  const fetchedCourse = useMemo(() => {
    ...
    return courseResponse.data?.data ?? courseResponse.data ?? courseResponse;
  }, [courseResponse]);
  ...
  useEffect(() => {
    if (isEdit) {
      if (fetchedCourse) {
        ...
        applyCourseData(item);
      }
    } else {
      applyCourseData(null);
    }
  }, [isEdit, item, fetchedCourse, applyCourseData]);
  ...
  const persistStagedFaqs = async (newCourseId) => {
    ...
    setStagedFaqs([]);
    queryClient.invalidateQueries(["course-faq-inline", newCourseId]);
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (response) => {
      notifySuccess(item ? "Course updated successfully" : "Course created successfully");
    ...
    onError: (error) => {
      notifyError(error?.response?.data?.message || "Failed to save course");
    },
  });
  ...
  const submitCourse = (values, saveWithDate = true) => {
    ...
    appendIfPresent("description", values.course_intro);
            ...
            accept="image/*"
                {...register("thumbnail", {
                  validate: (value) => {
                    ...
                    if (!item) {
                      return "Course icon is required";
                    }
                    return true;
                  },
                })}
        ...
        <section className="border rounded-lg p-4 space-y-6">
          {sections.map((section, index) => (
            ...
            <Button
              ...
              {section.section_key === "emi_financial_support" && (
                <div className="space-y-2">
                  <Label className="block mb-1 text-sm font-medium">
                    EMI & Financial Partners
                  </Label>
                  <MultiSelect
                    value={selectedEmiPartners}
                    onChange={(e) => setSelectedEmiPartners(e.value || [])}
                    options={emiPartners}
                    optionLabel="name"
                    optionValue="id"
