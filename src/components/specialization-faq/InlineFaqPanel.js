...
import {
  ...
  fetchSpecializationFaqsBySpecializationId,
  addSpecializationFaq,
  updateSpecializationFaq,
  deleteSpecializationFaq,
} from "@/lib/menuApi";
...
export default function SpecializationFaqInlinePanel({
  specializationId,
  specializationName,
  stagedFaqs,
  setStagedFaqs,
  ...
  const isExistingEntity = Boolean(specializationId);
  
  const { data: categoriesData, isLoading: isLoadingCategories, refetch: refetchCategories } = useQuery({
    queryKey: ["course-faq-inline-categories"],
    queryFn: () => fetchCourseFaqCategories({ page: 1, limit: 100 }),
  });
  ...
  const {
    ...
    queryKey: ["specialization-faq-inline", specializationId],
    queryFn: () => fetchSpecializationFaqsBySpecializationId(specializationId),
    enabled: isExistingEntity,
  });

  const remoteFaqs = faqsData?.data || [];
  ...
  const handleSaveFaq = async (values, { addAnother = false } = {}) => {
    ...
    if (!isExistingEntity) {
      if (editingFaq?._isRemote === false || editingFaq?.tempId) {
        ...
        notifySuccess("FAQ staged. It will be saved after the specialization is created.");
      }
      if (!addAnother) {
        handleCloseFaqForm();
      } else {
    ...
    try {
      if (editingFaq?._isRemote && editingFaq.id) {
        await updateSpecializationFaq(editingFaq.id, {
          ...finalPayload,
      ...
      queryClient.invalidateQueries(["specialization-faq-inline", specializationId]);
      if (!addAnother) {
        handleCloseFaqForm();
      } else {
        setEditingFaq(null);
  ...
  const handleDeleteFaq = async (faq) => {
    ...
    try {
      await deleteSpecializationFaq(faq.id);
  ...
  return (
    <div className="space-y-6 mt-4">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          {categories.length === 0 && (
            <p className="mt-2 text-xs text-destructive">No categories found. Please create categories first in the Course FAQs page.</p>
          )}
        </div>
        <div className="flex gap-2">
