    ...
    <div className={containerClassName}>
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          <input
            type="checkbox"
            id="news-faq-save-with-date"
            {...register("saveWithDate")}
            className="h-4 w-4"
          />
          <Label htmlFor="news-faq-save-with-date" className="cursor-pointer text-sm">
...
export default function NewsFaqInlinePanel({
  newsId,
  newsName,
  stagedFaqs,
  setStagedFaqs,
  ...
  const isExistingEntity = Boolean(newsId);

  const {
    data: faqsData,
    isLoading: isLoadingFaqs,
  } = useQuery({
    queryKey: ["news-faq-inline", newsId],
    queryFn: () => fetchNewsFaqsByNewsId(newsId),
  ...
  const handleSaveFaq = async (values, { addAnother = false } = {}) => {
    ...
    if (!isExistingEntity) {
      if (editingFaq?._isRemote === false || editingFaq?.tempId) {
        ...
        notifySuccess("FAQ staged. It will be saved after the news item is created.");
      }
      if (!addAnother) {
        handleCloseFaqForm();
      } else {
    ...
    try {
      if (editingFaq?._isRemote && editingFaq.id) {
        await updateNewsFaq(editingFaq.id, {
          ...finalPayload,
          news_id: newsId,
        });
      ...
      queryClient.invalidateQueries(["news-faq-inline", newsId]);
      if (!addAnother) {
        handleCloseFaqForm();
      } else {
        setEditingFaq(null);
